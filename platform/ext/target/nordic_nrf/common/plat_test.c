...
typedef int (*pal_uart_logger_t)(const char *fmt, ...);
pal_uart_logger_t pal_uart_get_logger(void)
{
    return tfm_log_printf;
}
...
static bool psa_scratch_initialized;
__ALIGN(4) uint8_t __psa_scratch[PSA_TEST_SCRATCH_AREA_SIZE];
